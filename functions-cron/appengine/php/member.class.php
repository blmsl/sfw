...
trait sfwMember
{
    ...
    public function saveDriveMember($member, $club, $memberList)
    {
        $birthday = false;
        if (isset($member[19])) {
        ...
        if (!key_exists($id, $memberList)) {
            ...
            echo "bereits vorhanden" . $memberList[$id]["id"] . "<br />";
            return array(
                'id' => $id,
                'data' => $data,
                'newEntry' => false
    ...
    {
        return DateTime::createFromFormat('Y-m-d', $birthday, date_default_timezone_get())
            ->diff(new DateTime('now', date_default_timezone_get()))
            ->y;
    }
